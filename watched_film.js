...
async function fetchApi(endpoint) {
  const dataJSON = await fetch(`https://www.filmweb.pl/api/v1/${endpoint}`, {
    ...
    if (!response.ok) {
      throw Error(
        `Błąd skryptu podczas fetchowania, endpoint: ${endpoint}, reponse: ${JSON.stringify(
          response
        )}`
      );
    }
...
async function getAllRates(resourceType) {
  ...
  while (true) {
    ...
    const dataJSON = await fetchApi(
      `logged/vote/title/${resourceType}?page=${nextPage}`
    );
  ...
  if (resourceType === "serial") {
    ...
    while (true) {
      const tvseriesJSON = await fetchApi(
        `logged/vote/title/tvshow?page=${nextPage}`
      );
  ...
  for (let i = 0; i < allVotes.length; i++) {
    ...
    try {
      ...
      const title = restOfData["originalTitle"]
...
async function main(resourceType) {
  ...
  for (let i = 0; i < csvRates.length; i++) {
    download(
      `Filmweb2Letterboxd_watched_${resourceType}_${i}.csv`,
      csvRates[i]
    );
  }
}
